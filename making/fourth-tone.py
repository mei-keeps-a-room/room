#!/usr/bin/env python3
"""Fourth tone — quiet. the water sits. Held low air + soft glass; leaf partial; no bloom glass."""
...
def water_hum(freq: float, t: float, t0: float, duration: float, water: int) -> float:
    """Sitting water — a soft held low that rises slowly and barely moves."""
    u = t - t0
    if u < 0 or u > duration:
        return 0.0
    # slow fade in, long hold, soft out
    attack = 1.4
    if u < attack:
        e = u / attack
    else:
...
def params_from(state: dict) -> dict:
    ...
    # high water softens overtones further; day 4 water=4 → quiet bloom
    overtone = max(0.28, 1.0 - 0.14 * water)
    sustain = 2.4 + 0.28 * (stem_h - 1) + 0.18 * (day - 1)

    # a little longer, quieter sitting
    duration = 7.0 + 0.35 * (day - 1) + 0.15 * stem_h + 0.10 * water
    duration = min(11.0, max(7.0, duration))

    # lower / duskier than third-tone's A4 / E4
    base_glass = 392.0 * pitch_shift   # G4-ish
    base_string = 293.66 * pitch_shift  # D4-ish
    hum = 98.0 * pitch_shift           # G2-ish sitting water
...
def sample_at(t: float, p: dict) -> float:
    ...
    # water sits first — low hum under everything
    x += water_hum(p["hum"], t, 0.15, p["duration"] - 0.4, p["water"])

    # one soft glass, later and quieter than third
    x += glass(g0, t, 1.10, dec * 1.15, ov)
    # string after a longer breath
    x += string(s0, t, 2.55, dec * 0.9)
    ...
    if p["blooms"] > 0:
        x += glass(g0 * 1.5, t, 4.2, dec * 0.9, ov) * 0.40

    # room air, slower
    x += 0.0028 * math.sin(2 * math.pi * 0.19 * t) * env_glass(t, 0.8, p["duration"] * 0.6)
    return x
