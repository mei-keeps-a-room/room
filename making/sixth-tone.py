#!/usr/bin/env python3
"""Sixth tone — a bloom. Second glass arrives over the sitting water."""
...
def params_from(state: dict) -> dict:
    ...
    # bloom day: glass a touch brighter, string holds the lengthen, second glass at 1.5x
    base_glass = 415.3 * pitch_shift   # Ab4-ish — open
    base_string = 311.13 * pitch_shift  # Eb4-ish
...
def sample_at(t: float, p: dict) -> float:
    ...
    x += string(s0, t, 2.05, dec * 1.05)
    # soft lengthen echo still there, quieter under the bloom
    x += string(s0 * 1.5, t, 3.20, dec * 0.7) * 0.32

    for i in range(p["leaves"]):
        pf = g0 * (2.0 + 0.08 * i)
        t0 = 3.35 + 0.5 * i
        x += soft_partial(pf, t, t0, dec * 0.85)

    # the bloom: second glass, clearer than the whisper on earlier days
    blooms = max(1, p["blooms"])
    for b in range(blooms):
        t0 = 4.15 + 0.85 * b
        x += glass(g0 * 1.5, t, t0, dec * 1.05, ov) * (0.72 - 0.08 * b)
        # a small bright partial riding the bloom
        x += soft_partial(g0 * 2.25, t, t0 + 0.35, dec * 0.7) * 0.55

    x += 0.0024 * math.sin(2 * math.pi * 0.19 * t) * env_glass(t, 0.8, p["duration"] * 0.6)
    return x
