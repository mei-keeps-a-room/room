#!/usr/bin/env python3
"""Ninth tone — a second stem arrives, another leaf. Bloom already open; a quieter presence beside."""
...
def thin_glass(freq: float, t: float, t0: float, decay: float) -> float:
    """Softer, thinner glass — fewer overtones, quieter — for the short new stem."""
    u = t - t0
    e = env_glass(u, 0.35, decay)
    if e < 1e-5:
        return 0.0
    s = math.sin(2 * math.pi * freq * u)
    s += 0.05 * math.sin(2 * math.pi * (freq * 2.01) * u)
    return e * s * 0.075
...
def soft_string(freq: float, t: float, t0: float, decay: float) -> float:
    """Quieter, thinner string — fewer harmonics — second stem's voice."""
    u = t - t0
    e = env_pluck(u, decay)
    if e < 1e-5:
        return 0.0
    s = 0.0
    for h in range(1, 4):
        amp = (1.0 / (h ** 1.7)) * math.exp(-0.9 * (h - 1) * u)
        s += amp * math.sin(2 * math.pi * freq * h * u)
    return e * s * 0.055
...
def params_from(state: dict) -> dict:
    ...
    short_h = min((int(s.get("h", 1)) for s in stems), default=1)
    n_stems = len(stems)
    leaves = sum(int(s.get("leaves", 0)) for s in stems)

    # tall stem still drops a little with height; second stem stays low/thin
    pitch_shift = 1.0 - 0.018 * (stem_h - 1)
    overtone = max(0.25, 1.0 - 0.14 * water)
    sustain = 2.55 + 0.33 * (stem_h - 1) + 0.15 * (day - 1)

    duration = 7.1 + 0.33 * (day - 1) + 0.20 * stem_h + 0.10 * water + 0.25 * max(0, n_stems - 1)
    duration = min(12.5, max(7.0, duration))

    # tall bloomed stem: Ab glass, E string (family of day 8)
    # second stem: softer, lower — C3-ish thin string + thin glass a fifth below
    base_glass = 415.3 * pitch_shift   # Ab4-ish — tall stem
    base_string = 329.63 * pitch_shift  # E4-ish
    short_string = 196.0 * pitch_shift  # G3-ish — quieter second presence
    short_glass = 246.94 * pitch_shift  # B3-ish thin glass for short stem
...
def sample_at(t: float, p: dict) -> float:
    ...
    # tall stem glass — present, not arriving
    x += glass(g0, t, 0.85, dec * 1.15, ov)
    # tall stem string
    x += string(s0, t, 2.10, dec * 1.05)

    # second stem arrives later, quieter — thin lower string + thin glass
    if p["n_stems"] > 1:
        x += soft_string(p["short_string"], t, 2.85, dec * 0.95)
        x += thin_glass(p["short_glass"], t, 3.35, dec * 0.90)
    ...
    # bloom already here — soft, present
    if p["blooms"] > 0:
...
def main() -> None:
    ...
    print(
        f"day={p['day']} water={p['water']} stem_h={p['stem_h']} short_h={p['short_h']} "
        f"n_stems={p['n_stems']} leaves={p['leaves']} blooms={p['blooms']} "
        f"dur={p['duration']:.2f}s glass={p['base_glass']:.1f}Hz "
        f"string={p['base_string']:.1f}Hz short={p['short_string']:.1f}Hz "
        f"hum={p['hum']:.1f}Hz -> {OUT.name}"
    )
