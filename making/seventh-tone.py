#!/usr/bin/env python3
"""Seventh tone — quiet. the water sits. Bloom held soft over deeper water."""
...
def water_hum(freq: float, t: float, t0: float, duration: float, water: int) -> float:
    """Sitting water — deeper hold on a high-water quiet day."""
    u = t - t0
    ...
    if u < attack:
        ...
        e = math.exp(-(u - attack) / (duration * 0.62))
    wobble = 1.0 + 0.0035 * math.sin(2 * math.pi * 0.09 * u)
    depth = 0.045 + 0.007 * min(water, 8)
...
def params_from(state: dict) -> dict:
    ...
    overtone = max(0.24, 1.0 - 0.145 * water)
    sustain = 2.55 + 0.30 * (stem_h - 1) + 0.17 * (day - 1)

    duration = 7.2 + 0.34 * (day - 1) + 0.18 * stem_h + 0.12 * water
    duration = min(12.0, max(7.0, duration))

    # quieter than bloom day: duskier glass, bloom glass held soft
    base_glass = 392.0 * pitch_shift   # G4-ish
    base_string = 293.66 * pitch_shift  # D4-ish
    hum = 92.5 * pitch_shift           # a touch lower than day 4
...
def sample_at(t: float, p: dict) -> float:
    ...
    # soft glass later, quieter sitting with the bloom already open
    x += glass(g0, t, 1.25, dec * 1.2, ov) * 0.88
    x += string(s0, t, 2.70, dec * 0.95)

    for i in range(p["leaves"]):
        pf = g0 * (2.0 + 0.08 * i)
        t0 = 3.55 + 0.55 * i
        x += soft_partial(pf, t, t0, dec * 0.82)

    # bloom still here — held, not arriving
    if p["blooms"] > 0:
        for b in range(p["blooms"]):
            t0 = 4.55 + 0.9 * b
            x += glass(g0 * 1.5, t, t0, dec * 1.1, ov) * (0.42 - 0.06 * b)
            x += soft_partial(g0 * 2.2, t, t0 + 0.4, dec * 0.75) * 0.35

    x += 0.0026 * math.sin(2 * math.pi * 0.17 * t) * env_glass(t, 0.9, p["duration"] * 0.65)
    return x
